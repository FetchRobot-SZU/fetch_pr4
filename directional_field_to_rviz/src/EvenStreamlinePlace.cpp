#include "directional_field_to_rviz/EvenStreamlinePlace.h"
#include "directional_field_to_rviz/GlView.h"
EvenStreamlinePlace::EvenStreamlinePlace(bool type, int initsize=300)	
{
		majororminor = type;
...
void EvenStreamlinePlace::setGlView(GlView *glview){
	m_glview=glview;
	quadmesh=m_glview->quadmesh;
}
...
void EvenStreamlinePlace::get_tenvec_quad(double cur_p[2], double vec[2])
{
	...
	m_glview->cal_eigen_vector_sym(ten, ev);
